...
.pg-current-tag{font-size:10px;color:var(--dsw-alias-brand-primary);border:1px solid var(--dsw-alias-brand-primary);border-radius:999px;padding:0 6px;line-height:16px;flex-shrink:0;}
...
.pg-dock{display:flex;align-items:center;gap:8px;padding:6px 4px;font-size:12px;}
.pg-dock-chip{display:inline-flex;align-items:center;gap:6px;background:var(--dsw-alias-bg-layer-1);border:1px solid var(--dsw-alias-border-l1);border-radius:999px;padding:3px 12px;}
...
.pg-dock-select{background:transparent;color:var(--dsw-alias-label-primary);border:1px solid var(--dsw-alias-border-l1);border-radius:999px;padding:2px 8px;font-size:12px;max-width:320px;outline:none;cursor:pointer;}
.pg-dock-select:focus{border-color:var(--dsw-alias-brand-primary);}
		...
		function apply(ctx) {
			...
			function ProjectGroupsPage() {
				...
				return React.createElement("div", { className: "pg-page" },
					...
					React.createElement("p", { className: "pg-hint" }, "一个项目组可包含多个项目文件夹。在同一个会话中，agent 可以直接读写组内任意文件夹的文件并运行命令，也可在组内快速切换当前文件夹。"),
						...
						: groups.map((g) => {
								...
								return React.createElement("div", { key: g.id, className: "pg-group" },
									...
									g.folders.length === 0
										...
										: g.folders.map((f) => {
												const isActive = f.id === store.activeFolderId;
												return React.createElement("div", { key: f.id, className: "pg-folder" },
													React.createElement("span", { className: "pg-dot" + (isActive ? " pg-dot-active" : "") }),
													...
													isActive ? React.createElement("span", { className: "pg-current-tag" }, "当前") : null,
													React.createElement("div", { className: "pg-folder-actions" },
														React.createElement("button", { className: "pg-btn pg-btn-sm" + (isActive ? " pg-btn-primary" : ""), disabled: busy || isActive, onClick: () => run(() => call("setActive", { folderId: f.id })) }, "设为当前"),
