...
export default {
  ...
  async apply(ctx) {
    ...
    function markDirty() {
      ...
      }).catch((error) => console.error('[dsh-ProjectModel] settings save failed:', error && error.message))
    }
    ...
    async function resolveFolder(args) {
      const folderId = args && typeof args.folderId === 'string' && args.folderId.length > 0 ? args.folderId : state.activeFolderId
      if (!folderId) {
        const lines = state.groups.length === 0
          ? '（尚无项目组，请先在“设置 → 项目组”中添加）'
          : state.groups.map((g) => g.name + ': ' + (g.folders.length === 0
              ? '（空）'
              : g.folders.map((f) => f.title + ' [' + f.path + '] id=' + f.id).join('；'))).join('\n')
        throw new Error('未指定文件夹。组内所有文件夹都可直接操作，无需切换：\n' + lines)
      }
    ...
    if (tools) {
      tools.register(defineTool({
        name: 'project',
        description: '项目组工具：把多个项目文件夹组成一个「项目组」，在同一个会话中跨全部文件夹工作——无需手动切换，直接对组内任意文件夹读写文件、列目录、运行命令。项目组在“设置 → 项目组”中维护。用法：先 list 查看项目组与文件夹及其 folderId；readFile/writeFile/listDir/run 都通过 folderId 指定目标文件夹（省略时用默认文件夹，若有）；path 一律是相对该文件夹的路径（如 package.json、src/main.ts）。也支持管理动作 createGroup/addFolder/removeFolder/deleteGroup，setActive 仅设置默认文件夹（可选，不影响直接操作其他文件夹）。动作名同时接受别名：read、write、list_dir、set_active 等。未配置项目组时应提示用户先到“设置 → 项目组”添加文件夹。',
        parameters: {
          action: { type: 'string', required: true, description: '动作：list | readFile | writeFile | listDir | run（也接受 read/write/list_dir 等别名；管理动作 createGroup/addFolder/removeFolder/deleteGroup/setActive）' },
          folderId: { type: 'string', description: '目标文件夹 id（list 查看）；省略时用默认文件夹（若有）' },
          path: { type: 'string', description: '相对目标文件夹的文件/目录路径（readFile/writeFile/listDir 使用）' },
      ...
      const disposer = systemPrompt.context({
        ...
        text: () => {
          ...
          const folderLine = g.folders.length === 0
            ? '（空）'
            : g.folders.map((x) => x.title + ' ' + x.path + (x.id === state.activeFolderId ? ' ←默认' : '')).join('；')
          return '项目组：' + g.name + '（默认文件夹：' + (f ? f.folder.title + ' ' + f.folder.path : '未设置，可直接指定任意文件夹') + '）\n组内文件夹（均可直接操作）：' + folderLine
        },
      })
      if (disposer) ctx.effect(() => disposer)
    }

    console.log('[dsh-ProjectModel] ready with', state.groups.length, 'group(s)')
  },
}
